...
char *welcome(char* hello, char* name) {
	...
	/*Checks if combined length of "Hello {Name}" is too long for the MAX LIMIT*/
	if((length(name) + length(hello) + 1) > MAX_LIMIT){
		return "welcome error: name too long\n";
	}
	else{
		/*Copies/Concatenates the message array*/
...
void reverse(char* txt, char* result) {
	/*i holds the current position being copied from in txt, j holds the position being copied to in result*/
	int i = length(txt) - 1;
	int j = 0;
	while(i >= 0){
		result[j] = txt[i];
...
int vowels(char* txt) {
	int vowel = 0;
	int i = 0;
	/*Checks each character with an if statement that contains any vowel (caps or not) as a fulfilling condition*/
	while(txt[i] != '\0'){
...
int quartile(char* name) {
	//This function does not check for upper or lowercase since it appears that the main() function already capitalizes the first character of name with the "toupper" function
	...
	if(1 <= q && q <= 6){
		...
		q = 4;
	} else{ //Produces error for values outside the bounds of A-Z
		printf("quartile error: Name contains non-alphabetic value\n");
		q = 0;
	}
	return q;
}
